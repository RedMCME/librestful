...
class RequestThreadPool implements RequestThread{
    ...
    private function addWorker() : void{
        $this->workers[] = new DefaultRequestThread($this->notifier, $this->bufferSend, $this->bufferRecv);
    }

    public function join() : void{
        foreach($this->workers as $worker) $worker->join();
    }

    public function stopRunning() : void{
        foreach($this->workers as $worker) $worker->stopRunning();
    }
